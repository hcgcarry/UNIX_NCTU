...
void* memset(void *s,int chr, size_t size) {
	char *ptr = (char *) s;
	while(size-- > 0) *ptr++ = chr;
	return s;
}
int convertNumToMask(int shiftVal){
	return 1<< (shiftVal-1);
}
...
int sigaddset(sigset_t *set, int signo){
	set->__val[0] |= convertNumToMask(signo);
	return 0;
}
...
int sigprocmask(int how, const sigset_t *set, sigset_t * oldset){
	long ret =sys_rt_sigprocmask(how,set, oldset, sizeof(oldset));
	WRAPPER_RETval(int);
}
